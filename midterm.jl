...
df = missing ## REPLACE THIS WITH YOUR CODE
...
begin
	...
	function check_get_teams(x::Vector{String})
		if length(x) != 50
			return keep_working()
		end
		if !issorted(x)
...
begin
	...
	function check_get_team_ids(x::Dict{String,Int})
		if length(x) != 50
			return keep_working()
		end
